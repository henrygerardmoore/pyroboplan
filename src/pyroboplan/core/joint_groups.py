...
class JointGroup:
    ...
    def set_joint_state(self, state):
        if len(state) != len(self.joint_indices):
            raise IndexError(
                f"length of input state ({len(state)}) is not the same as the size of joint model {self.name} ({len(self.joint_indices)})"
            )
...
class JointGroupManager:
    ...
    def make_joint_group(self, joint_group_name, joint_names: list[str]):
        if joint_group_name in self.joint_groups:
            raise IndexError(
                f"{joint_group_name} already refers to a joint group. Delete and re-create the joint group if you want to reuse this name"
            )
        joint_ids = []
        for joint_name in joint_names:
            ...
            if id > self.model.nq:
                ...
                raise IndexError(
                    f"Could not find joint {joint_name} in model. Possible names are {names}"
                )
            joint_ids.append(id)

        joint_ids.sort()
        self.joint_groups[joint_group_name] = JointGroup(
            joint_group_name, self, self.model, joint_ids
        )
